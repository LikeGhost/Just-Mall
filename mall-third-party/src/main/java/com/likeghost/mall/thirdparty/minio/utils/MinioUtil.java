package com.likeghost.mall.thirdparty.minio.utils;

import com.likeghost.mall.thirdparty.minio.config.MinioConfig;
import io.minio.*;
...
public class MinioUtil {
    ...
    public boolean deletes(String bucketName, List<String> objectNames) {
        try {
            ...
            return deleteErrorNames.size() == 0;
        } catch (Exception e) {
            log.error("errorMsg={}", e);
            return false;
        }
     ...
     * @return 文件访问链接
     */
    public String getPreSignedObjectUrl(Method method, String bucketName, String remoteFileName, long timeout) {
        try {
            return minioClient.getPresignedObjectUrl(
                    GetPresignedObjectUrlArgs.builder()
                            ...
                            .expiry((int) timeout, TimeUnit.SECONDS)
                            .build());
        } catch (Exception e) {
            log.error("errorMsg={}", e);
            return null;
     ...
     * @return
     */
    public String getPreSignedObjectUrl(Method method, String remoteFileName) {
        return getPreSignedObjectUrl(method, minioConfig.getDefaultBucketName(), remoteFileName, 60 * 60 * 24 * 7);
    }
     ...
     * @return
     */
    public String putPreSignedObjectUrl(String remoteFileName) {
        return getPreSignedObjectUrl(Method.PUT, remoteFileName);
    }
     ...
     * @return
     */
    public String getPreSignedObjectUrl(String remoteFileName) {
        return getPreSignedObjectUrl(Method.GET, remoteFileName);
    }


    public String getEndpoint(String bucketName) {

        String endpoint = "http://" + minioConfig.getEndpoint();
        Integer port = minioConfig.getPort();
        if (port != null) {
            endpoint += ":" + port;
        }
